...
@humi.route('/week', methods=['GET'])
@jwt_required()
def get_humidity_by_week():
    ...
    end_date_obj = dateutil.get_datetime_by_utc_isoformat(target_date)
    if not end_date_obj:
    ...
    query = f"""
            ...
            from(bucket: "smarthome")
                ...
                |> filter(fn: (r) => r._measurement == "GatewayData" and r._field == "humidity" and r.mac_address == "{gateway_id}")
                |> window(every: 1d)
                |> mean()
