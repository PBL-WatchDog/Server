...
def get_door_by_day():
    ...
    last_value = influx_query.get_last_value_for_SensorData_by_range([0, start_standard_date_obj.isoformat()], gateway_id, [target_device], "Contact")
    query = f"""
            from(bucket: "smarthome")
                |> range(start: {start_standard_date_obj.isoformat()}, stop: {end_date_obj.isoformat()})
                |> filter(fn: (r) => r._measurement == "SensorData")
                |> filter(fn: (r) => r.mac_address == "{gateway_id}" and r.Device == "{target_device}" and r._field == "Contact")
    ...
    for i, v in enumerate(data):
        ...
        if first:
            ...
            if prev_first is None:
                if last_value.get(target_device) == ON_FLAG:
