...
@door.route('/week', methods=["GET"])
@jwt_required()
def get_door_by_week():
    ...
    if not device_list:
        return jsonify({"msg":"no device"}), 204
    times = influx_query.get_last_time_for_SensorData(gateway_id, device_list, "Contact")
    ...
    for device_id, end_time in times.items():
        ...
        query = f"""
                from(bucket: "smarthome")
                    ...
                    |> filter(fn: (r) => r.mac_address == "{gateway_id}" and r.Device == "{device_id}" and r._field == "Contact")
