...
@oxy.route('/range', methods=['GET'])
@jwt_required()
def get_oxymeter_by_range():
    ...
    tags = {'mac_address' : gateway_id, 'Device' : 'oxymeter'}
    ...
    formatted_results = []
    for item in result:
        formatted_results.append({'date': dateutil.get_utc_isoformat(item['_time']), 'spo2': item['spo2'], 'bpm':item['bpm']})
